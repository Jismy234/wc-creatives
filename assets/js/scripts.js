...
function decreaseCount(event, element) {
  ...
  function updateSubtotal(input) {
    ...
    var subtotalElement = row.querySelector('[data-subtotal]');
  ...
  function updateCartTotals() {
    ...
    subtotals.forEach(function(subtotalElement) {
      cartSubtotal += parseFloat(subtotalElement.getAttribute('data-subtotal'));
    });
    var discount = calculateDiscount(cartSubtotal);
    var total = cartSubtotal - discount;
    document.getElementById('cart-subtotal').textContent = `₹${cartSubtotal.toFixed(2)}`;
    document.getElementById('cart-total').textContent = `₹${total.toFixed(2)}`;
  }
  
  function calculateDiscount(cartSubtotal) {
    // Example: 10% discount
  ...
  function proceedToCheckout() {
    ...
    productRows.forEach(function(row) {
      var product = row.querySelector('[data-column="Product"] h6').textContent;
      var price = row.querySelector('[data-price]').getAttribute('data-price');
      var quantity = row.querySelector('input').value;
      var subtotal = row.querySelector('[data-subtotal]').getAttribute('data-subtotal');
      cartDetails += `Product: ${product}, Price: ₹${price}, Quantity: ${quantity}, Subtotal: ₹${subtotal}\n`;
    });
